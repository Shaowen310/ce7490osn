...
class PartitionPlan:
    def __init__(self, n_partitions, user_capacity, partition_capacity):
        ...
        self.user2replica = np.empty((self.ucap, self.pcap), dtype=np.bool)

    def partition_least_masters(self):
        master_count = np.sum(self.user2primary, axis=0)
        master_count[~self.palloc] = np.nan
    ...
    def partition_add_slave(self, partition_id, user_id):
        self.ualloc[user_id] = True
        self.palloc[partition_id] = True
        self.user2replica[user_id, partition_id] = True

    def partition_ids(self):
        return np.nonzero(self.palloc)
